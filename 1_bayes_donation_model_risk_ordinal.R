# 1_bayes_donation_model_risk_ordinal.R
# version with ordinal donation outcome
# September 2021
library(dplyr)
library(R2WinBUGS)
...
categories = 'four' 
# results are saved
outfile = 'results/bugs_ordinal_model_risk.RData'
if(categories == 'four'){
  outfile = 'results/bugs_ordinal_model_risk_four.RData'
}
...
# make risk categories and country numbers
risk_labels = c('Unanswered','Unwilling to risk','Willing to risk')
data = mutate(data, 
	risk_group = case_when(
		is.na(willing_risk) ~ 1,
		!is.na(willing_risk) & willing_risk <= 5 ~ 2,
		!is.na(willing_risk) & willing_risk > 5 ~ 3),
	risk_group = factor(risk_group, levels=1:3, labels=risk_labels),
	risk_group = relevel(risk_group, ref='Unwilling to risk'), # make this the reference category
    countrynum = as.numeric(as.factor(country)))
countries = unique(data$country)
n.country = n_distinct(data$country) # number of countries
risk_labels = c('Unwilling to risk','Unanswered','Willing to risk') # match re-ordering of reference category
...
formula = 'donate_ordinal ~ -1 + risk_group'
for.model = data.frame(model.matrix(as.formula(formula), data=data))
names(for.model) = c('risk_unwilling','risk_no_answer','risk_willing')
...
model.file = 'bugs_donation_risk_ordinal_fixed.txt'
type = 'fixed'
n_indep = length(risk_labels)
...
N = nrow(for.model) # 
X = as.matrix(select(for.model, starts_with('risk_'))) # extract variables
X[,1] = 1 # reference group is unwilling
P = ncol(X)
...
# create external text file with bugs model
model.file = 'bugs_donation_risk_ordinal_variable.txt'
type = 'random'
source('99_make_bugs.R')
...
###
# save results
save(countries, n.country, risk_labels, bugs.results, bugs.results.fixed, file=outfile)
