# 1_bayes_donation_model_income_ordinal.R
...
categories = 'four' 
# results are saved
outfile = 'results/bugs_ordinal_model_income.RData'
if(categories == 'four'){
  outfile = 'results/bugs_ordinal_model_income_four.RData'
}
...
formula = 'donate_ordinal ~ -1 + incomecat'
...
income_labels = c('Low','High','Unanswered')

### Part 1: fixed effects model

# create external text file with bugs model
model.file = 'bugs_donation_income_ordinal_fixed.txt'
type = 'fixed'
n_indep = length(income_labels)
...
N = nrow(for.model) # 
X = as.matrix(select(for.model, starts_with('i_'))) # extract variables
X[,1] = 1 # reference group is low
P = ncol(X)
...
# create external text file with bugs model
model.file = 'bugs_donation_income_ordinal_variable.txt'
type = 'random'
source('99_make_bugs.R')
...
###
# save results
save(countries, n.country, income_labels, bugs.results, bugs.results.fixed, file=outfile)
