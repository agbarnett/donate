# 1_bayes_donation_model_age_ordinal.R
...
formula = 'donate_ordinal ~ -1 + factor(age_group)'
...
N = nrow(for.model) # 
X = as.matrix(select(for.model, starts_with('age_'))) # extract variables
X[,1] = 1 # reference group is youngest age
P = ncol(X)
