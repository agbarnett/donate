...
```{r}
# run the function to create rows of statistics
educ = cell_rows(indata=data, var='education_level')
ideology = cell_rows(indata=data, var='ideologycat')
altruism = cell_rows(indata=data, var='altruism')
income = cell_rows(indata=data, var='incomecat')
gender = cell_rows(indata=data, var='sex')
agecat = cell_rows(indata=data, var='age_group')
risks = cell_rows(indata=data, var='risk_group')
mega_table = bind_rows(educ, ideology, altruism, income, gender, agecat, risks) %>%
  rename('Variable' = 'var')
header = data.frame()
ftab = flextable(mega_table) %>%
  add_header(Variable='' , category='' ,
             `Should donate less than 10%`='Willing to donate' ,
             `Should donate 10%`='Willing to donate' ,
             `Should donate more than 10%`='Willing to donate' ,
             `Any`='Willing to donate' ,
             `Should not donate`='' ,
             `Do not know`='' ,
             `Prefer not to say`='') %>%
  merge_v(j=1) %>%
