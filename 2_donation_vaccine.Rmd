...
#all_results_fixed = process_results_fixed(in.results=bugs.results.fixed, max.category.num=4) 
all_results = process_results_random(in.results=bugs.results, max.category.num=4) %>%
  rename('enum' = 'num')
```
...
ftab = flextable(dic_table) %>%
  theme_box() %>%
  autofit()  %>%
  colformat_num(2, digits=1) %>%
  colformat_num(3, digits=0)
ftab
```

pD is the expected number of parameters. The difference in the DIC is `r round(diff(dic_table$DIC))` which is strong evidence that the random model is better. 

### Table of estimates (education)

```{r}
# 
stats_education = group_by(all_results, countrynum, overall, enum) %>%
...
index = elabels %in% c('Unknown','Missing')
if(any(index)){colours[index] = missing_colour} # consistent colour for missing
# forest plots
fplot = ggplot(data=for_plot, aes(x=countrynum, y=mean, ymin=lower, ymax=upper, col=factor(enum)))+
  geom_point(size=2)+
...
fplot
jpeg('figures/education_donation.jpg', width=5.5, height=6.5, units='in', res=500)
print(fplot)
invisible(dev.off())
```
...
```{r}
# 
stats_politics = group_by(all_results, countrynum, p_num, overall) %>%
  summarise(mean = mean(p),
            lower = quantile(p, 0.025), # Bayesian CI
            upper = quantile(p, 1 - 0.025)) %>%
  ungroup()
...
colours = RColorBrewer::brewer.pal(4, 'Dark2')
index = poll_labels %in% c('Unknown','Missing')
if(any(index)){colours[index] = missing_colour} # consistent colour for missing
# forest plots
fplot = ggplot(data=for_plot, aes(x=countrynum, y=mean, ymin=lower, ymax=upper, col=factor(p_num)))+
  geom_point(size=3)+
...
fplot
jpeg('figures/politics_donation.jpg', width=5, height=5.5, units='in', res=500)
print(fplot)
invisible(dev.off())
```
...
all_results = process_results_random(in.results=bugs.results, max.category.num=2) %>%
  rename('gnum' = 'num')
...
pD is the expected number of parameters. The difference in the DIC is `r round(diff(dic_table$DIC))` which is moderate evidence that the random model is better. 

### Table of estimates (gender)

```{r}
# 
stats_gender = group_by(all_results, countrynum, gnum, overall) %>%
  summarise(mean = mean(p),
            lower = quantile(p, 0.025), # Bayesian CI
            upper = quantile(p, 1 - 0.025)) %>%
  ungroup()
...
# forest plots
fplot = ggplot(data=for_plot, aes(x=countrynum, y=mean, ymin=lower, ymax=upper, col=factor(gnum)))+
  geom_point(size=3)+
  geom_errorbar(width=0, size=1.05)+
...
## construct probability of supporting vaccine donation from chains
all_results = process_results_random(in.results=bugs.results, max.category.num=3) %>%
  rename('age_num' = 'num')
```
...
pD is the expected number of parameters. The difference in the DIC is `r round(diff(dic_table$DIC))` which is means the fixed model is better. So there's no evidence that the age effect varies by country. 

### Table of estimates (age)

```{r}
# 
stats_age = group_by(all_results, countrynum, overall, age_num) %>%
  summarise(mean = mean(p),
            lower = quantile(p, 0.025), # Bayesian CI
            upper = quantile(p, 1 - 0.025)) %>%
...
index = age_labels %in% c('Missing','Unknown')
if(any(index)){colours[index] = missing_colour} # consistent colour for missing
# forest plots
fplot = ggplot(data=for_plot, aes(x=countrynum, y=mean, ymin=lower, ymax=upper, col=factor(age_num)))+
  geom_point(size=2)+
  geom_errorbar(width=0, size=1.05)+
...
fplot
jpeg('figures/age_donation.jpg', width=4.5, height=5.5, units='in', res=500)
print(fplot)
invisible(dev.off())
```
...
We created a three category variable for altruism based on the donation amount. The first category combined "missing", "do not know" and "prefer not to answer". The next two categories were "Meagre" which was below the median donation amount for that country, and "Generous" which was on or above the median donation amount for that country. Stratifying by country for this variable is necessary because each country had different amounts and units of currency.
...
pD is the expected number of parameters. The difference in the DIC is `r round(diff(dic_table$DIC))` which is strong evidence that the simpler fixed model is better. 

### Table of estimates (altruism)

```{r}
# 
stats_altruism = group_by(all_results, countrynum, overall, enum) %>%
  summarise(mean = mean(p),
            lower = quantile(p, 0.025), # Bayesian CI
            upper = quantile(p, 1 - 0.025)) %>%
...
colours = RColorBrewer::brewer.pal(3, 'Dark2')
index = altruism_labels %in% c('Missing','Unknown')
if(any(index)){colours[index] = missing_colour} # consistent colour for missing
# forest plots
fplot = ggplot(data=for_plot, aes(x=countrynum, y=mean, ymin=lower, ymax=upper, col=factor(enum)))+
  geom_point(size=2)+
...
fplot
jpeg('figures/altruism_donation.jpg', width=5.5, height=6.5, units='in', res=500)
print(fplot)
invisible(dev.off())
```
...
This variable was collected on a range of 0 (completely unwilling) to 10 (Very willing), we divided this into two groups defining those with a score of 0 to 5 as "Unwilling" and those with a score of 6 to 10 as "Willing". We added a third group of "Missing". 
...
pD is the expected number of parameters. The difference in the DIC is `r round(diff(dic_table$DIC))` which is strong evidence that the more complex random model is better. 

### Table of estimates (risks with health)

```{r}
# 
stats_risk = group_by(all_results, countrynum, overall, enum) %>%
  summarise(mean = mean(p),
            lower = quantile(p, 0.025), # Bayesian CI
            upper = quantile(p, 1 - 0.025)) %>%
...
for_plot = mutate(stats_risk, 
                  ...
                  countrynum = ifelse(enum==3, countrynum+0.15, countrynum))
colours = RColorBrewer::brewer.pal(3, 'Dark2')
index = risk_labels %in% c('Missing','Unknown')
if(any(index)){colours[index] = missing_colour} # consistent colour for missing
# forest plots
fplot = ggplot(data=for_plot, aes(x=countrynum, y=mean, ymin=lower, ymax=upper, col=factor(enum)))+
  geom_point(size=2)+
...
fplot
jpeg('figures/risk_donation.jpg', width=5.5, height=6.5, units='in', res=500)
print(fplot)
invisible(dev.off())
```
...
The table shows the differences in the overall probability of a willingness to donate vaccine for the three risk groups and the reference group of "No answer". The Bayesian p-value is the probability that the probability to donate vaccine is higher in each of the two groups compared with the reference group. Hence a very small or large p-value is evidence of a difference, and a p-value close to 0.5 is evidence of little difference between the two groups. 
...
pD is the expected number of parameters. The difference in the DIC is `r round(diff(dic_table$DIC))` which indicates that the simpler fixed model is better. . 

### Table of estimates (income)

```{r}
# 
stats_risk = group_by(all_results, countrynum, overall, inum) %>%
  summarise(mean = mean(p),
            lower = quantile(p, 0.025), # Bayesian CI
            upper = quantile(p, 1 - 0.025)) %>%
...
colours = RColorBrewer::brewer.pal(3, 'Dark2')
index = ilabels %in% c('Missing','Unknown')
if(any(index)){colours[index] = missing_colour} # consistent colour for missing
# forest plots
fplot = ggplot(data=for_plot, aes(x=countrynum, y=mean, ymin=lower, ymax=upper, col=factor(inum)))+
  geom_point(size=2)+
  geom_errorbar(width=0, size=1.05)+
...
fplot
jpeg('figures/income_donation.jpg', width=5.5, height=6.5, units='in', res=500)
print(fplot)
invisible(dev.off())
```
...
fplot = ggplot(data=for_plot, aes(x=countrynum, y=mean, ymin=lower, ymax=upper, col=factor(label)))+
  ...
  geom_text(data=points_text, aes(x=countrynum, y=lower, ymin=lower, ymax=lower, col=factor(label), label=reflabel), hjust = 1.1)+ # hjust puts labels on left
  geom_point(size=2)+
  geom_errorbar(width=0, size=1.05)+
...
fplot
jpeg('figures/multiple.jpg', width=8.5, height=6.5, units='in', res=500)
print(fplot)
invisible(dev.off())
## to here
# check that CIs correspond with estimates of alphas
```
...
# Those with a missing ideology

In this section we look at those with a missing ideology.
...
The table shows the numbers and percents that are willing or not to donate the vaccine. The results are stratified by country and whether the ideology was missing or not. 

The interesting comparison is to look at the percent willing in each country for those with missing ideology or not. In every country the percent willing to donate the vaccine is much lower in those who did not answer the ideology question.

## Lottery vignette

The histogram below shows the answers to the lottery vignette for those with a missing ideology or not. 
...
stats = mutate(data, 
  ...
  ungroup() %>%
  mutate(facet = ifelse(Missing=='Yes', 'Ideology missing', 'Ideology complete'))
#
