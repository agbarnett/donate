# 1_bayes_donation_model_education_ordinal.R
...
categories = 'four' 
# results are saved
outfile = 'results/bugs_ordinal_model_education.RData'
if(categories == 'four'){
  outfile = 'results/bugs_ordinal_model_education_four.RData'
}
...
formula = 'donate_ordinal ~ -1 + factor(education_level)'
...
elabels = c('Primary or less','Secondary','University','Unanswered')

### Part 1: fixed effects model

# create external text file with bugs model
model.file = 'bugs_donation_education_ordinal_fixed.txt'
type = 'fixed'
n_indep = length(elabels)
...
N = nrow(for.model) # 
X = as.matrix(select(for.model, starts_with('e_'))) # extract variables
X[,1] = 1 # reference group is primary or less
P = ncol(X)
...
# create external text file with bugs model
model.file = 'bugs_donation_education_ordinal_variable.txt'
type = 'random'
source('99_make_bugs.R')
...
###
# save results
save(countries, n.country, elabels, bugs.results, bugs.results.fixed, file=outfile)
