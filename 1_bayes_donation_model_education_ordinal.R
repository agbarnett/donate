...
cat('model{
  ...
  for (i in 1:N){
    ...
    for (k in 1:(ncat-1)){
      ...
      beta[countrynum[i],4]*X[i,4] - C[countrynum[i],k];
    }
    # Calculate probabilities
    pi[i,1] <- 1 - Q[i,1]; # Pr(cat=1) = 1 - Pr(cat>1);
    for (k in 2:ncat-1) {
...
###
# save results
save(countries, n.country, elabels, bugs.results, bugs.results.fixed, file='results/bugs_ordinal_model_education.RData')
