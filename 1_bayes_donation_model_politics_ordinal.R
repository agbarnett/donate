# 1_bayes_donation_model_politics_ordinal.R
...
categories = 'four' 
# results are saved
outfile = paste('results/bugs_ordinal_model_politics.RData')
if(categories == 'four'){
  outfile = paste('results/bugs_ordinal_model_politics_four.RData')
}
...
formula = 'donate_ordinal ~ -1 + factor(group)'
...
model.file = 'bugs_donation_ideology_ordinal_fixed.txt'
type = 'fixed'
n_indep = length(poll_labels)
...
# create external text file with bugs model
model.file = 'bugs_donation_ideology_ordinal_variable.txt'
type = 'random'
source('99_make_bugs.R')
...
###
# save results
save(countries, n.country, poll_labels, bugs.results, bugs.results.fixed, file=outfile)
